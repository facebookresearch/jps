...
def get_tricks(cards, strain, first=0):
    ...
    tricks = torch.from_numpy(tricks).long().reshape_as(cards).to(cards.device)
    return tricks


@hydra.main(config_path="conf/config_belief_mean_trick.yaml", strict=True)
def main(args):
    ...
    for epoch in range(args.num_epoch):
        ...
        for batch_idx in range(args.epoch_len):
            ...
            tricks = get_tricks(batch.d["cards"], strain, 0)

            obs = dict(s=batch.d["s"], tricks=tricks)
            belief_loss, prob_trick = belief_model.loss(obs)
            ...
            belief_stat["prob_trick"].feed(prob_trick)
