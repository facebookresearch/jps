...
def evaluate_models(epoch, args, factory, actor_gen, actor_gen_baseline, env_actor_gen, stat):
    trainer = Trainer()
...
def main(args):
    ...
    if args.eval_only:
        ...
        stat.reset()
        evaluate_models(args.eval_fake_epoch, args, factory, actor_gen, actor_gen_baseline, env_actor_gen, stat)
    ...
    if args.negative_momentum is not None:
        # Negative momentum!
        optim = SGDChanged(trainable_params(), lr=args.lr, momentum=args.negative_momentum)
    else:
        optim = torch.optim.Adam(trainable_params(), lr=args.lr, eps=args.eps)

    # Create replay_buffer.
    print("Create replay buffer")
    replay_buffer = rela.PrioritizedReplay2(
    ...
    while replay_buffer.size() < actual_burn_in:
        log.info(f"warming up replay buffer: {replay_buffer.size()} / {actual_burn_in}")
        time.sleep(1)
    ...
    for epoch in range(args.num_epoch):
        ...
        for batch_idx in range(args.epoch_len):
            ...
            loss.backward()
            g_norm = torch.nn.utils.clip_grad_norm_(trainable_params(), args.grad_clip)
            optim.step()
        ...
        # [TODO] Fix tachometer
        log.info('\n' + tachometer.lap(None, replay_buffer, args.epoch_len * args.batchsize, 1))
        if stopwatch is not None:
            log.info('\n' + stopwatch.summary())

        save_filename = f"agent-{epoch}.pth" 
        agent.save(save_filename)
        log.info(f"Save to {os.path.join(os.getcwd(), save_filename)}, md5: {get_md5(save_filename)}")

        trainer.on_epoch_finish(agent)

        context.pause()
        evaluate_models(epoch, args, factory, actor_gen, actor_gen_baseline, env_actor_gen, stat)
        context.resume()
