...
Transition Transition::makeBatch(std::vector<Transition> transitions, 
                                 int batchdim, 
  ...
  for (size_t i = 0; i < transitions.size(); i++) {
    ...
    utils::tensorVecDictAppend(ds, transitions[i].d);
  }

  Transition batch;
  batch.d = utils::tensorDictJoin(ds, batchdim);
  ...
  if (device != "cpu") {
    ...
    batch.d = utils::tensorDictApply(batch.d, toDevice);
  }
  return batch;
}
...
Transition Transition::index(int i) const {
  std::cout << "Calling Transition::index" << std::endl;
  Transition element;
  for (auto& name2tensor : d) {
    element.d.insert({name2tensor.first, name2tensor.second[i]});
  }
  return element;
}
*/
...
Transition Transition::padLike() const {
  Transition pad;
  pad.d = utils::tensorDictZerosLike(d);
  return pad;
}

TorchJitInput Transition::toJitInput(const torch::Device& device) const {
  TorchJitInput input;
  input.push_back(utils::tensorDictToTorchDict(d, device));
  return input;
}
